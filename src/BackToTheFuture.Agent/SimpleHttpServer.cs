...
namespace BackToTheFuture.Agent
{
    class SimpleHTTPServer
    {
        ...
        public SimpleHTTPServer(string path, int port, ISiteTarget site)
        {
        ...
        /// </summary>
        /// <param name="path">Directory path to serve.</param>
        public SimpleHTTPServer(string path, ISiteTarget site)
        {
            //get an empty port
        ...
        private void Process(HttpListenerContext context)
        {
            ...
            string resourceName = context.Request.Url.AbsolutePath;
            Console.WriteLine(String.Format("Incoming request: {0}.", resourceName));

            List<string> defaultContentNames = new List<string>(new string[] { "index.htm", "index.html", "default.htm", "default.html"});
            List<string> resourceNames = new List<string>();

           
            resourceNames.Add(resourceName);
            Uri uri = new Uri("file://" + resourceName);

            var absolutPath = uri.AbsolutePath;
            foreach(var defaultContentName in defaultContentNames)
            {
                resourceNames.Add(string.Format("{0}/{1}", absolutPath, defaultContentName));
            }
            ...
            try
            {
                TargetResource resoruceToServer = null;

                foreach (var resource in resourceNames)
                {
                    resoruceToServer =  _site.GetByName(new Uri("file://" + resource).AbsolutePath.Substring(1));
